      ...
      <div className="my-12">
        ...
        <div className="flex items-center justify-between mt-2 mb-4">
          ...
          <div>
            <Link
              to="/adduser"
              className="py-2 px-6 bg-blue-800 text-white font-medium rounded-xl cursor-pointer transition-all duration-300 hover:bg-blue-900"
            >
              Add User
            </Link>
          </div>
          ...
          <tbody className="bg-slate-200">
            {users.length >= 1 ? (
              users.map((user, index) => (
                <tr
                  key={index}
                  className={`${index % 2 === 0 ? "bg-slate-200" : "bg-slate-100"} hover:bg-slate-300 duration-300 ease-in`}
                >
                  <Td
                    text={`#${user.id < 10 ? `0${user.id}` : `${user.id}`}`}
                  />
                  <Td text={user.nome} />
                  <Td text={user.apelido} />
                  <Td text={user.email} />
                  <Td
                    text={`${new Date().getFullYear() - user.anoNascimento} anos`}
                  />
                  <td className=" p-2 w-60">
                    <div className="w-full flex gap-2 justify-center items-center">
                      <Link
                        to={`/detailsuser/${user.id}`}
                        className="bg-blue-600 text-white flex cursor-pointer items-center justify-center p-1.25 rounded-sm transition-all duration-300 hover:bg-blue-700"
                      >
                        <Eye size={18} />
                      </Link>
                      <Link
                        to={`/edituser/${user.id}`}
                        className="bg-green-600 text-white flex items-center cursor-pointer justify-center p-1.25 rounded-sm transition-all duration-300 hover:bg-green-700"
                      >
                        <Pencil size={18} />
                      </Link>
                      <button
                        onClick={() => deleteUsers(user.id)}
                        className="bg-red-600 text-white flex items-center cursor-pointer justify-center p-1.25 rounded-sm transition-all duration-300 hover:bg-red-700"
                      >
                        <Trash2 size={18} />
                      </button>
                    </div>
